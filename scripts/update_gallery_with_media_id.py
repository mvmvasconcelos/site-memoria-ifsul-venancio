...
def main() -> None:
    ...
    with app.app_context():
        columns = {column["name"] for column in inspect(db.engine).get_columns("gallery_item")}
        if "media_id" not in columns:
            print("Coluna gallery_item.media_id não existe no schema atual. Nada para atualizar.")
            return
        ...
        rows = db.session.execute(text("SELECT id, image_path, media_id FROM gallery_item ORDER BY id ASC")).mappings().all()
        ...
        for row in rows:
            ...
            if mapped_id is None:
                stats["not_found"] += 1
                print(f"MISS Item {item_id}: {image_path}")
                continue

            db.session.execute(
                text("UPDATE gallery_item SET media_id = :media_id WHERE id = :item_id"),
                {"media_id": int(mapped_id), "item_id": int(item_id)},
            )
            stats["updated"] += 1
            print(f"UPDATE Item {item_id} -> media_id={mapped_id}")
